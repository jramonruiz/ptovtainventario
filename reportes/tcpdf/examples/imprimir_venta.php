...
$rsttd = mysql_query("select tv.id_venta,cu.nombre_usuario,cs.descripcion_sucursal,cc.descripcion_caja,tv.total_venta,tv.nombre_cliente,tv.tipo_operacion,tv.referencia,tv.descuento,tv.total_pagar,cs.calle,cs.numero_exterior,cs.colonia,cs.municipio,cs.estado,cs.telefono,cs.rfc,tv.cambio_venta,tv.folioempresa from tventas tv inner join cusuarios cu on tv.id_usuario=cu.id_usuario inner join csucursales cs on cu.id_sucursal=cs.id_sucursal inner join ccajas cc on cu.id_caja=cc.id_caja where tv.id_venta=$idultimaventa");
if ($rowttd = mysql_fetch_row($rsttd)) {
   ...
   $nombre_cliente= utf8_encode($rowttd[5]);
   ...
   $rfc= utf8_encode($rowttd[16]);
   $cambio_venta= utf8_encode($rowttd[17]);   
   $folioempresa= utf8_encode($rowttd[18]);   
}
...
//$pdf->AddPage('P', 'A4');
         ...
         <tr>
            <td align="center" border="0" colspan="4"><b>DATO 1 DE LA EMPRESA</b></td>
         </tr>
         <tr>
            <td align="center" border="0" colspan="4"><b>DOMICILIO DE LA EMPRESA</b></td>
          </tr>
          <tr>
            <td align="center" border="0" colspan="4"><b>TELEFONO</b></td>
          </tr>
          <tr>
            <td align="center" border="0" colspan="4"><b>FECHA:'.$hoy.'</b></td>
          </tr>
          <tr>
            <td align="center" border="0" colspan="4"><b>CLIENTE:'.$nombre_cliente.'</b></td>
          </tr>
          <tr>
            <td align="center" border="0" colspan="4"><b>FOLIO:'.$folioempresa.'</b></td>
          </tr>
          <tr>
            <td align="center" border="0" colspan="4">&nbsp;</td>
          </tr>
         <tr>
         <td align="left" border="0"><b>CANT</b></td>
          <td align="left" border="0"><b>PRODUC</b></td>
         ...
         $Resultado2=mysql_query("select * from tproductos_venta where id_venta=$idultimaventa");  
            ...
            {
            $table5.= '<tr><td align="left" colspan="4">'.$MostrarFila2['descripcion_producto'].'</td></tr>';
            $table5.='<tr><td align="left" colspan="2">'.$MostrarFila2['cantidad'].'</td>';
            $table5.= '<td align="right">'.$MostrarFila2['precio_venta'].'</td>';
            $table5.= '<td align="right">'.$MostrarFila2['subtotal'].'</td>';     
            $table5.= '</tr>';    
            $total_productos_vendidos=$total_productos_vendidos+$MostrarFila2['cantidad'];    
            }     
         ...
         $table5.= '<tr border="0">
            <td colspan="4" align="right">TOTAL: $ '.$total_venta.'</td>
         </tr>';
         ...
         $Resultado2pagosventa=mysql_query("select tpv.id_pago_venta,tmp.desc_metodo_pago,tpv.monto,tpv.referencia from tpagos_venta tpv inner join tmetodos_pago tmp on tpv.id_tipo_pago=tmp.id_metodo_pago where tpv.id_venta=$idultimaventa");  
            ...
            {
            $table5.= '<tr><td align="right" colspan="4">'.$MostrarFila2pagosventa['referencia'].' '.$MostrarFila2pagosventa['desc_metodo_pago'].' $ '.utf8_encode($MostrarFila2pagosventa['monto']).'</td>';
            $table5.= '</tr>';         
            } 

         $table5.= '<tr border="0">
            <td colspan="4" align="right">Cambio: $ '.$cambio_venta.'</td>
         </tr>';
...
$rsavitkt = mysql_query("select id_aviso,aviso from tavisos_ticket where id_aviso=1");
if ($rowavitkt = mysql_fetch_row($rsavitkt)) {
   $id_aviso= utf8_encode($rowavitkt[0]);
   $aviso= $rowavitkt[1];
}


    $table5.= '<tr border="0">
            <td colspan="4" align="left">Productos vendidos:'.$total_productos_vendidos.'</td>
         </tr>';
         ...
         <tr border="0">
            <td colspan="4" align="left">'.$aviso.'</td>
         </tr>';

...
$rsttd = mysql_query("select tv.id_venta,cu.nombre_usuario,cs.descripcion_sucursal,cc.descripcion_caja,tv.total_venta,tv.nombre_cliente,tv.tipo_operacion,tv.referencia,tv.descuento,tv.total_pagar from tventas tv inner join cusuarios cu on tv.id_usuario=cu.id_usuario inner join csucursales cs on cu.id_sucursal=cs.id_sucursal inner join ccajas cc on cu.id_caja=cc.id_caja where tv.id_venta=$id_venta");
...
//$pdf->AddPage('P', 'A4');
			...
			$Resultado2pagosventa=mysql_query("select tpv.id_pago_venta,tmp.desc_metodo_pago,tpv.monto,tpv.referencia from tpagos_venta tpv inner join tmetodos_pago tmp on tpv.id_tipo_pago=tmp.id_metodo_pago where tpv.id_venta=$id_venta");	

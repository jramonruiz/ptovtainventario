...
class MYPDF extends TCPDF {
	...
	public function Footer() {
		...
		$this->Cell(0, 10, 'Reporte de valor de inventario', 'T', false, 'C');				
		$this->SetY(-15);
		// Set font
		//$this->SetFont('helvetica', 'I', 8);
		// Page number
...
//$pdf->AddPage('P', 'A4');
			...
			$table1.= '<table><thead border="0">
       		<tr>
				<td align="center" border="0" colspan="6">
					...
					<b>Reporte valor del inventario</b><br>
					<b>FECHA:'.$hoy.'</b><br>
			...
			<tr>
				<td align="left">No</td><td align="left">CODIGO</td><td align="left">DESCRIPCION</td><td align="right">EXISTENCIA</td><td align="right">PRECIO DE VENTA</td><td align="right">IMPORTE</td>
			</tr>
			<tr align="center" border="0">
				<td colspan="6" style="border-bottom:solid 1px color:#000000;">&nbsp;</td>
			</tr>';
			...
			$i=1;
				...
				if($ordenarpor==1)
					{	
						$Resultado2=mysql_query("select codigo_barras,descripcion,cantidad_existencia,precio_venta from cproductos where id_sucursal=$id_sucursal order by id_proveedor");
					}
				else
					{
						$Resultado2=mysql_query("select codigo_barras,descripcion,cantidad_existencia,precio_venta from cproductos where id_sucursal=$id_sucursal order by id_departamento");
					}
				...
				while($MostrarFila2=mysql_fetch_array($Resultado2))
				{
					...
					$precio_venta=utf8_encode($MostrarFila2['precio_venta']);

					$importe=$cantidad_existencia*$precio_venta;

					$total_valor=$total_valor+$importe;

					$totalproductos=$totalproductos+1;
				
			  		$table1.='<tr><td align="left">'.$i.'</td><td align="left">'.$codigo_barras.'</td>'.'<td align="left">'.$descripcion.'</td>'.'<td align="right">'.$cantidad_existencia.'</td>'.'<td align="right">'.$precio_venta.'</td>'.'<td align="right">'.$importe.'</td>';
					  $table1.= '</tr>';
				...
				<td colspan="6">Total de productos: '.$totalproductos.'<br>Total del valor del inventario: $ '.$total_valor.'</td>
			</tr>';
...
$pdf->Output('reportevalorinventario.pdf', 'I');
...
$rs = mysql_query("SELECT id_producto,codigo_barras,descripcion,precio_compra,cantidad_existencia,precio_venta FROM cproductos where descripcion='$txtproductodesc'");
if ($row = mysql_fetch_row($rs)) {
$id_producto_buscar = trim($row[0]);
$codigo_barras = trim($row[1]);
$descripcion = trim($row[2]);
$precio_compra = trim($row[3]);
$cantidad_existencia = trim($row[4]);
$precio_venta = trim($row[5]);
}
...
if($id_producto_buscar!=0)
	{
		...
		echo "1,".$id_producto_buscar.",".$codigo_barras.",".$descripcion.",".$precio_compra.",".$cantidad_existencia;		
	}
else
	{
		echo "0";

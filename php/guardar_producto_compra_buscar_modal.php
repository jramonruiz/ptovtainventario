...
$txtid_productomodcom=utf8_decode($_POST['txtid_productomodcom']);
$txtcantidad_productomodcom=utf8_decode($_POST['txtcantidad_productomodcom']);
...
{
	$subtotal=$txtnuevo_precio_compra_modcom;	
	$importe=$subtotal*$txtcantidad_productomodcom;
}
else
{
	$porcentaje_descuento=$txtdescuento_productomodcom/100;
	$monto_descuento=$txtnuevo_precio_compra_modcom*$porcentaje_descuento;
	$subtotal=$txtnuevo_precio_compra_modcom-$monto_descuento;
	$importe=$subtotal*$txtcantidad_productomodcom;
}
...
$sql= mysql_query("insert into tproductos_compra(descripcion_producto,precio_compra,cantidad_comprada,subtotal,id_usuario,id_producto,ultimo_precio_compra,id_sucursal,descuento,importe) values(\"$txtdescripcion_producto\",$txtnuevo_precio_compra_modcom,$txtcantidad_productomodcom,$subtotal,$id_usuario,$txtid_productomodcom,$txtprecio_compra_ultimo,$id_sucursal,$txtdescuento_productomodcom,$importe)");

/************** ACTUALIZANDO INVENTARIO ********/

$cantidad_existencia_nueva=$txtcantidad_existencia_producto_buscar+$txtcantidad_productomodcom;

$sql2= mysql_query("update cproductos set cantidad_existencia=".$cantidad_existencia_nueva.",precio_compra=".$txtnuevo_precio_compra_modcom.",precio_venta=".$txtprecio_venta_modcom." where id_producto=".$txtid_productomodcom."");

...
if($idcuentapagar>0)
{
$query = "select id_cuenta_pagar,id_proveedor,nombre_proveedor,documento,total_pagar,faltante_pagar,pagada,descuento_pronto_pago,pagado_dinero from tcuentas_pagar where id_cuenta_pagar=$idcuentapagar";
...
<form id="frm_nuevo_abono" name="frm_nuevo_abono" action="" method="post"> 
  ...
  <input type="hidden" id="txtid_cuenta_pagar" name="txtid_cuenta_pagar" value="<?php echo $id_cuenta_pagar; ?>" /> 
  <input type="hidden" id="txtfaltante_pagar" name="txtfaltante_pagar" value="<?php echo $faltante_pagar; ?>" /> 
    <div class="wrapper">
                ...
                <form role="form">
                  <div class="box-body">
                     <div class="form-group">
                      <h4><strong>Proveedor:</strong> <?php echo $nombre_proveedor; ?>, <strong>Numero de documento o factura:</strong> <?php echo $documento; ?></h4><br>
                      <h4><strong>Monto: $ </strong><?php echo $pagado_dinero; ?>, <strong>Descuento por pronto pago: $ </strong><?php echo $descuento_pronto_pago; ?>, <strong>Total a pagar: $ </strong><?php echo $total_pagar; ?></h4><br>
                      </div>
                    <div class="form-group">
                    ...
                    <tbody>
                        <?php 
$listado=  mysql_query("select tap.id_abono_cpagar,tap.fecha,tap.id_tipo_pago,tap.importe,tap.comentario,tap.pagado,tmp.desc_metodo_pago from tabonos_cpagar tap inner join tmetodos_pago tmp on tap.id_tipo_pago=tmp.id_metodo_pago order by tap.fecha");
                   while($reg=  mysql_fetch_array($listado))
                   {
                               $id_abono_cpagar=utf8_encode($reg['id_abono_cpagar']);
...
?>        
                  ...
                  <div class="box-footer">
					<button type="button" class="btn btn-primary" onClick="javascript:alta_abono();">Guardar</button>
                  </div>
                  <?php
                      }
                  ?>
